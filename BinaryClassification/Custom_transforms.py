...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb  7 12:24:02 2020

@author: mpostigo

Auxiliary functions for the images tranforms
...
class applyMaskTransform (object):
    ...
    def __call__(self, sample):
        ...
        return {'image': res, 'mask': res_mask, 'label': label, 'ga':sample['ga'], 'la':sample['la']}
...
class MyRotationTransform (object):
    ...
    def __call__(self, sample):
        ...
        return {'image': rot_img, 'mask': rot_mask, 'label': label, 'ga':sample['ga'], 'la':sample['la']}
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        return {'image': torch.from_numpy(np.flip(image,axis=0).copy()) ,
                ...
                'ga':torch.from_numpy(np.asarray(sample['ga'])),
                'la':torch.from_numpy(np.asarray(sample['la']))}
...
class MyHorizontalFlipTransform(object):
    ...
    def __call__(self, sample):
        ...
        return {'image': img, 'mask': mask, 'label': sample['label'], 'ga':sample['ga'], 'la':sample['la']}
...
class RandomIncreaseBrightness (object):
    ...
    def __call__(self, sample):
        ...
        return {'image': img, 'mask': sample ['mask'], 'label': sample['label'], 'ga':sample['ga'], 'la':sample['la']}